...
cursor.execute("CREATE TABLE if not exists signup (username varchar(30) DEFAULT NULL,mobile varchar(12) DEFAULT NULL,email varchar(50) NOT NULL,address varchar(75) DEFAULT NULL,password text,PRIMARY KEY (email),UNIQUE KEY unique_email (email),KEY email (email),KEY email_2 (email),KEY idx_username (username))")
cursor.execute("CREATE TABLE if not exists adminsignup (name varchar(30) DEFAULT NULL,mobile bigint NOT NULL,email varchar(50) DEFAULT NULL,password varchar(40) DEFAULT NULL,PRIMARY KEY (mobile),UNIQUE KEY email (email))")
cursor.execute("CREATE TABLE if not exists additems (itemid varchar(30) NOT NULL,name varchar(30) DEFAULT NULL,discription longtext,qty varchar(20) DEFAULT NULL,category enum('electronics','grocery','fashion','home') DEFAULT NULL,price varchar(30) DEFAULT NULL,PRIMARY KEY (itemid))")
cursor.execute("CREATE TABLE  if not exists orders (order_id bigint NOT NULL AUTO_INCREMENT,itemid varchar(30) NOT NULL,item_name longtext NOT NULL,qty int DEFAULT NULL,total_price bigint DEFAULT NULL,user varchar(100) DEFAULT NULL,PRIMARY KEY (order_id),KEY user (user),KEY itemid (itemid),CONSTRAINT orders_ibfk_1 FOREIGN KEY (user) REFERENCES signup (username),CONSTRAINT orders_ibfk_2 FOREIGN KEY (itemid) REFERENCES additems (itemid))")
@app.route('/')
def base():
...
def updateproducts(itemid):
    if session.get('admin'):
        print(itemid)
        cursor=mydb.cursor()
        cursor.execute('select name,description,qty,category,price from additems \
...
def orders():
    if session.get('user'):
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('selecct*from orders where user=%s',[user])
        data=cursor.fetchall()
        cursor.close()
        return render_template('orderdisplay.html',data=data)
    else:
        return redirect(url_for('login'))
@app.route('/searcch',methods=['GET','POST'])
def search():
    if request.method=='POST':
